#ifndef RAYCASTCOLORMIP_H
#define RAYCASTCOLORMIP_H

#include <string>

std::string raycastColorMIP = R"(
/////////////////////////////////////////////////////////////////////////
// Front to Back MIP rendering for Color image              //
/////////////////////////////////////////////////////////////////////////
__kernel void renderMIPColor_F2B(	__global uint *d_output,
...

{
    ...
    //float focalLength = fmin(ratioX, ratioY) * 0.5f * 2.0f / tan(3.141592653589793f/8.0f); //= 1/2 * borderLength / tan(FOV/2)
    ...
    for(uint i=0; i<2000 && travel>0.0; i++, pos+=step, travel-=TSTEP) // nb iteration depends on stepsize, should be large enough
    {
            ...
            pos.z>myParam->clipPlaneCenter[4] && pos.z<myParam->clipPlaneCenter[5])
        {
            ...
            temp.x = fmax(temp.x, lutB.x);
            temp.y = fmax(temp.y, lutG.y);
            temp.z = fmax(temp.z, lutR.z);
            temp.w = fmax(temp.w, fmax(fmax(lutR.w, lutG.w), lutB.w));
            ...
            if(sample.w*myParam->isoValue[3] > 0.5f)
            {
                ...
                temp = temp + (1.0f - temp.w)*addBlinnPhongShadingX(isoVal, volume, pos, lightPosition, eyePosition)*isoVal.w;
                temp.w = fmin(temp.w + 0.96f*isoVal.w, 0.96f);
            ...
            float colorNorm = sqrt(dot(color,color)/3.0f);

            if( colorNorm>0.1f && bFirstElt)
            {
                float4 newPos = pos*2.0f - 1.0f;
                newPos.x *= alpha;
                newPos.y *= -beta;
