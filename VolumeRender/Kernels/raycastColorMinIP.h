...
#ifndef RAYCASTCOLORMINIP_H
#define RAYCASTCOLORMINIP_H

#include <string>

std::string raycastColorMinIP = R"(
__kernel void renderMinIPColor_F2B(	__global uint *d_output,
...

{
    ...
    for(uint i=0; i<2000 && travel>0.0; i++, pos+=step, travel-=(float)TSTEP) // nb iteration depends on stepsize, should be large enough
    {
            ...
            pos.z>myParam->clipPlaneCenter[4] && pos.z<myParam->clipPlaneCenter[5])
        {
            ...
            temp.x = fmin(temp.x, lutB.x);
            temp.y = fmin(temp.y, lutG.y);
            temp.z = fmin(temp.z, lutR.z);
