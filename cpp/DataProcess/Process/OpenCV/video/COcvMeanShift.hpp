...
class COcvMeanShift : public CVideoTask
{
    ...
    private:
        ...
        void    setRoiToTrack()
        {
            ...
            if(items.size() == 0)
            {
                qCritical().noquote() << QObject::tr("MeanShift algorithm requires rectangular object");
                return;
            }

            if(items.size() > 1)
                qWarning().noquote() << QObject::tr("MeanShift algorithm can only track one rectangular object");

            auto pItem = std::dynamic_pointer_cast<CProxyGraphicsRect>(items.back());
            if(pItem == nullptr)
            {
                qCritical().noquote() << QObject::tr("MeanShift algorithm can only track rectangular object");
                return;
            }
