#ifndef CFEATUREPROCESSIO_H
#define CFEATUREPROCESSIO_H

#include "DataProcessGlobal.hpp"
#include "Protocol/CProtocolTaskIO.h"
#include "Main/CoreTools.hpp"

/** @file CFeatureProcessIO.hpp */
...
class CFeatureProcessIOBase: public CProtocolTaskIO
{
    public:
        ...
        virtual ~CFeatureProcessIOBase(){}

        virtual CFeatureProcessIOBase&  operator=(const CFeatureProcessIOBase& io)
        {
...
/**
 * @ingroup groupCore
 * @brief The CFeatureProcessIO class defines an input or output for a workflow task dedicated to numeric values management.
 * @details This class is designed to handle numeric values as input or output of a workflow task. A CFeatureProcessIO<T> instance can be added as input or output to a CProtocolTask or derived object.
 ...
 */
template<class Type>
class CFeatureProcessIO : public CFeatureProcessIOBase
{
    public:

        using FeatureVector = std::vector<Type>;
        using VectorOfFeatureVector = std::vector<std::vector<Type>>;
        using FeatureProcessIOPtr = std::shared_ptr<CFeatureProcessIO>;

        /**
         * @brief Default constructor
         */
        CFeatureProcessIO() : CFeatureProcessIOBase()
        {
         ...
         */
        CFeatureProcessIO(const CFeatureProcessIO& io) : CFeatureProcessIOBase(io)
        {
        ...
        virtual ~CFeatureProcessIO(){}

        /**
         * @brief Assignement operator
         */
        virtual CFeatureProcessIO&  operator=(const CFeatureProcessIO& io)
        {
            CFeatureProcessIOBase::operator=(io);
            m_values = io.m_values;
            m_valueLabels = io.m_valueLabels;
            m_headerLabels = io.m_headerLabels;
            return *this;
         ...
         * @return CFeatureProcessIO smart pointer (std::shared_ptr).
         */
        FeatureProcessIOPtr             clone()
        {
            return std::static_pointer_cast<CFeatureProcessIO>(cloneImp());
        }
    ...
    private:
        ...
        {
            return std::shared_ptr<CFeatureProcessIO>(new CFeatureProcessIO(*this));
        }
...
class DATAPROCESSSHARED_EXPORT CFeatureProcessIOFactory: public CProtocolTaskIOFactory
{
    public:

        CFeatureProcessIOFactory()
        {
            m_name = "CFeatureProcessIO";
        }
        ...
        {
            Q_UNUSED(dataType);
            return std::make_shared<CFeatureProcessIO<double>>();
        }
};

#endif // CFEATUREPROCESSIO_H
