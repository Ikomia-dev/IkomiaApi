...
#if defined(_WIN32)
std::map<int, Device> CWindowsDeviceEnumerator::getVideoDevicesMap() {
    return getDevicesMap(CLSID_VideoInputDeviceCategory);
}

std::map<int, Device> CWindowsDeviceEnumerator::getAudioDevicesMap() {
    return getDevicesMap(CLSID_AudioInputDeviceCategory);
}

// Returns a map of id and devices that can be used
std::map<int, Device> CWindowsDeviceEnumerator::getDevicesMap(const GUID deviceClass)
{
...
std::string CWindowsDeviceEnumerator::ConvertBSTRToMBS(BSTR bstr)
{
    int wslen = ::SysStringLen(bstr);
    return ConvertWCSToMBS((wchar_t*)bstr, wslen);
}

std::string CWindowsDeviceEnumerator::ConvertWCSToMBS(const wchar_t* pstr, long wslen)
{
